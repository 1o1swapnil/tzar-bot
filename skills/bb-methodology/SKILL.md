---
name: bb-methodology
description: Bug-bounty master orchestrator — 5-phase non-linear hunting workflow plus critical-thinking framework; routes to other skills. Use at the start of a hunt or when unsure what to do next.
---
...
**Command**: the `reconnaissance` skill on target.com (Tzar-Bot has no recon slash-command)
...
**Pre-report gate:**
```
Run validate-finding.py (7-Question Gate)
+-- All 7 pass? -> Write report
+-- Any fail? -> KILL the finding. Don't waste time.
+-- Borderline? -> run the validate-finding.py pre-check for quick go/no-go
```
...
- [ ] Record finding with `session-memory.py` for hunt memory
...
- [ ] Log findings with `session-memory.py`
...
- **`triage-validation`** — When a finding completes Phase 4 and is about to be written up. Workflow primitive: Phase 5 explicitly calls `validate-finding.py` (the 7-Question Gate); only findings that pass all 7 questions get handed off to `report-writing`.
...
## Operator Notes (engagement-derived)
