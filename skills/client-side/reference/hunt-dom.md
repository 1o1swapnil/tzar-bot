...
grep -rnE "addEventListener\(\s*['\"]message['\"]|onmessage\s*=" $OUTPUT_DIR/recon/ --include="*.js" 2>/dev/null \
  | grep -vE "\.origin\b" 
...
grep -rnE "postMessage\([^,]+,\s*['\"]\*['\"]\)" $OUTPUT_DIR/recon/ --include="*.js" 2>/dev/null
```
...
```bash
grep -rnE "dangerouslySetInnerHTML|v-html=|\[innerHTML\]=|\.html\(" $OUTPUT_DIR/recon/ --include="*.js" 2>/dev/null
...
grep -rnE "angular|vue|handlebars|mustache|nunjucks|alpinejs|\bv-|ng-app" $OUTPUT_DIR/recon/ --include="*.js" 2>/dev/null | head
