---
name: hunt-ntlm-info
description: NTLM/Negotiate info disclosure — anonymous Type-2 challenge leaks AD domain, forest, and hostname via AV_PAIRS. Use when the target offers NTLM or Negotiate auth.
...
3. **Use a keep-alive raw socket, not Python requests / curl one-shot.** Most HTTP libraries close the connection between the Type-1 send and Type-2 reception. Use one of:
   ...
   - `ncat`/`openssl s_client` interactive session (keeps the socket open between sends)
   - Python raw `socket` + `ssl.wrap_socket` (see Payload section) — preferred for executor agents
...
**Raw HTTP/1.1 keep-alive request (Burp Repeater or `ncat`/raw socket — recommended for full Type-2 capture):**
```
