---
name: hunt-websocket
description: WebSocket attacks — cross-site WebSocket hijacking (CSWSH), missing origin check, auth-over-WS bypass, message tampering. Use when the app uses WebSockets.
...
grep -rE "new WebSocket|io\(|io\.connect|socket\.io|new SockJS|signalr|Phoenix\.Socket|wss?://" \
  $OUTPUT_DIR/recon/ --include="*.js" 2>/dev/null | \
...
grep -iE "socket|/ws\b|websocket|stream|realtime|live|chat|events|/cable|/signalr|notifications" \
  $OUTPUT_DIR/recon/urls.txt | sort -u
