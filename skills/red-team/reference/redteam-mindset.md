...
- **Mode selection (coordinator)** — Once mindset is loaded, confirm the engagement mode (red-team vs WAPT, blackbox vs greybox) before routing to platform-specific skills. Engagement flow: red-team mindset triggered → confirm engagement mode (`bug-bounty` vs red-team vs pentest per project memory) → the coordinator mounts the right cluster (M365 / SharePoint / VPN / vCenter / APK).
