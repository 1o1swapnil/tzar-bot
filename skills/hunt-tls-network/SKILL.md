...
description: TLS / network-layer issues — weak ciphers, cert misconfig, CT-log subdomain discovery, dangling-CNAME / subdomain takeover. Use during recon and network testing.
...
curl -s "https://crt.sh/?q=%25.$TARGET&output=json" | \
  ...
  sed 's/\*\.//g' | sort -u > $OUTPUT_DIR/recon/ct-subdomains.txt

echo "[+] CT subdomains found: $(wc -l < $OUTPUT_DIR/recon/ct-subdomains.txt)"

# Compare with existing subdomain list
comm -23 <(sort $OUTPUT_DIR/recon/ct-subdomains.txt) \
         <(sort $OUTPUT_DIR/recon/subdomains.txt 2>/dev/null) | head -20
...
done < $OUTPUT_DIR/recon/ct-subdomains.txt | tee $OUTPUT_DIR/recon/cname-map.txt
